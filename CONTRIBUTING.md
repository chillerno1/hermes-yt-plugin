...
node .github/scripts/check-settings.mjs
```

CI runs exactly these six. `main` is protected, so everything lands through a
pull request.
...
without the leading `v`; for example, tag `v0.4.0` must contain
`const VERSION = '0.4.0'`.
